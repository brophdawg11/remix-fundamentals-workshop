...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  let numItems = cartItems.reduce((acc, ci) => acc + ci.quantity, 0);
  return json({
...
export default function App() {
  ...
  let addingQuantity = navigation.formData?.get("quantity");
  let optimisticNumItems =
    addingQuantity != null
      ? data.numItems + Number(addingQuantity)
      : data.numItems;
...
function DelayedSpinner() {
  ...
  React.useEffect(() => {
    ...
    if (navigation.state !== "idle" && !isAddingToBag) {
